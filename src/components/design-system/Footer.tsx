...
export default function Footer() {
  ...
  return (
    <footer className="ink-t sa-footer" style={{ marginTop: 96, background: 'var(--sa-bg)' }}>
      <style>{`
        .sa-footer-mobile { display: none; }
        @media (max-width: 640px) {
          .sa-footer { margin-top: 48px !important; }
          .sa-footer-desktop { display: none !important; }
          .sa-footer-mobile { display: block !important; }
        }
      `}</style>

      {/* Desktop / tablet footer */}
      <div className="sa-footer-desktop">
      ...
      {/* Mobile footer — purpose-built, not the desktop one reflowed */}
      <div className="sa-footer-mobile">
        <FooterMobile lang={lang} copy={copy} />
      </div>
    </footer>
  );
}
...
function FooterMobile({
  ...
  return (
    <div style={{ padding: '36px 20px 24px' }}>
      ...
      {/* Links — 2-col grid keeps it compact instead of a long stack */}
      <div
        className="rule-t"
        style={{
          marginTop: 26,
      ...
      >
        <FooterCol title={lang === 'fr' ? 'Produit' : 'Product'} labels={copy.footerProduit} items={LINKS.produit} />
        <FooterCol title={lang === 'fr' ? 'Ressources' : 'Resources'} labels={copy.footerRessources} items={LINKS.ressources} />
        <div style={{ gridColumn: '1 / -1' }}>
          <FooterCol title={lang === 'fr' ? 'Agence' : 'Agency'} labels={copy.footerAgence} items={LINKS.agence} />
        </div>
      </div>

      {/* Bottom — powered by + legal, given its own framed block so it reads
          intentional rather than cramped. */}
      <div
        className="ink-t"
        style={{ marginTop: 26, paddingTop: 20, display: 'flex', flexDirection: 'column', gap: 16 }}
      >
        <a
          href="https://pixelab.ch"
          target="_blank"
          rel="noopener noreferrer"
          aria-label="Pixelab"
          style={{ display: 'inline-flex', alignItems: 'center', gap: 10, color: 'var(--sa-ink)' }}
        >
          <span
            className="mono"
            style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--sa-ink-4)' }}
          >
            {copy.poweredBy}
          </span>
          <svg
        ...
        >
          <span>{copy.footerMeta[0]} · {copy.footerMeta[1]}</span>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
            <span style={{ width: 7, height: 7, background: 'var(--sa-red)', borderRadius: '50%' }} />
            {copy.footerMeta[2]}
          </span>
        </div>
      </div>
