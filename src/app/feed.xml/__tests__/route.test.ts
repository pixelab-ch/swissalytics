 ...
 *
 * PAYLOAD_URL is unset under vitest, so the loader's hub source short-circuits
 * to [] and the feed is built from the real FR MDX files — deterministic, no network.
 */
describe('GET /feed.xml', () => {
  let posts: ArticleMeta[];

  beforeAll(async () => {
    posts = await blog.listArticles('fr');
  });
